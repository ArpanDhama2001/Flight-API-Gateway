...
async function create(data) {
  try {
    ...
    user.addRole(role); // addRole() is sequelize many-to-many association helper method
    return user;
  } catch (error) {
...
async function addRoletoUser(data) {
  try {
    ...
    if (!role) {
      throw new AppError(
        "No role found for the given role name",
        StatusCodes.NOT_FOUND
      );
    }
    user.addRole(role);
...
async function addAdminRoleToUser(id) {
  try {
...
async function addFlightCompanyRoleToUser(id) {
  try {
    const user = await userRepo.get(id);
    const role = await roleRepo.getRoleByName(
      Enums.USER_ROLES_ENUMS.FLIGHT_COMPANY
    );
    if (!role) {
      throw new AppError(
        "Flight Company role not found",
        StatusCodes.NOT_FOUND
      );
    }
    user.addRole(role);
...
async function isFlightCompany(id) {
  try {
    ...
    const flightCompanyRole = await roleRepo.getRoleByName(
      Enums.USER_ROLES_ENUMS.FLIGHT_COMPANY
    );
